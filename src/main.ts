...
const fn = async () => {
    ...
    const topSecretcards = _.map(await utilObj.getBestCardAppreciation(6, 72, Rarity.SecretRare, paramCardType, 500, 20), (c) => ({ id: c.productId, set: c.set }))
    ...
    util.displayChanges(_.map(topCards, 'id'), [], 10, 20)
    console.log('\nTop Secret Rare Cards:')
    util.displayChanges(_.map(topSecretcards, 'id'), [], 10, 20)
    console.log('\n\nLatest Set Ultra Rares: ')
    util.displayChanges(_.map(latestSetUltraRare, 'id'), [], 10, 20)
    console.log('\n\nLatest Set Secret Rares: ')
    util.displayChanges(_.map(latestSetSecretRare, 'id'), [], 10, 20)
}
fn()